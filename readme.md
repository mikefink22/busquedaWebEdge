...
Antes de ejecutar el script, **verificá que la ruta de Edge sea correcta** en tu sistema:
...
2. Navegá a `C:\Program Files` o `C:\Program Files (x86)`
3. Buscá la carpeta `Microsoft\Edge\Application`
4. Si la ruta es diferente, editá la variable `ruta_edge` en `main.py` (línea 24)

---

## Uso y Ejecución

Podés correr el programa de dos maneras:
...
- O simplemente cerrá la ventana de la consola/terminal

---

## Lanzador Automático (.bat)

Como el archivo `.bat` está incluido en el `.gitignore` por seguridad de rutas, debés crearlo localmente si querés ejecutar el script con un solo clic:

### Pasos

1. Creá un archivo llamado `CorrerBusquedas.bat` en la raíz del proyecto
...
- Verificá que la ruta de Edge sea correcta (ver sección "Configuración Inicial")
- Asegurate de que Edge esté instalado en tu sistema
